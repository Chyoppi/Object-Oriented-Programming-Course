...
class Graph:
    ...
    def calculate_travel_cost(self, city_names):
        """
        Calculates the total travel cost (length) for a given path in the graph.
        :param city_names: List of city names representing the path.
        :return: Total travel cost if the path is valid, or None if the path is invalid.
        """
        total_cost = 0
        ...
        for i in range(len(city_names) - 1):
            ...
            # Ensure both cities exist in the graph
            if current_city not in self.__vertices or next_city not in self.__vertices:
                print(f"Invalid path: {current_city} or {next_city} does not exist in the graph.")
                return None

            # Find the edge weight between the current and next city
            current_vertex = self.__vertices[current_city]
            for transition in current_vertex.transitions:
                ...
                print(f"Invalid path: No direct connection between {current_city} and {next_city}.")
                return None

        return f"{round(total_cost, 1)} €"
...
#Lambda function to calculate travel cost
calculate_travel_cost_lambda = lambda graph, path: round(sum(
    next(
        (transition.weight for transition in graph._Graph__vertices[path[i]].transitions
         if transition.target.name == path[i + 1]),
        float('inf')  # Return infinity if no direct connection exists
    )
...
print(f"Path {valid_path_3} is valid: {graph.is_valid_path(valid_path_3)}")  # Expected: True
...
# Test the calculate_travel_cost method
path_1 = ["Turku", "Rauma"] #Returns 85,6 €
path_2 = ["Turku", "Helsinki"]  # Invalid path (no direct connection)

# Calculate travel costs using the method
cost_1 = graph.calculate_travel_cost(path_1)
cost_2 = graph.calculate_travel_cost(path_2)

#Lambda function
cost_1_lambda = calculate_travel_cost_lambda(graph, path_1)
cost_2_lambda = calculate_travel_cost_lambda(graph, path_2)

# Print results
print(f"Travel cost for path {path_1} (method): {cost_1}")  # Expected: Total cost of the valid path
print(f"Travel cost for path {path_2} (method): {cost_2}")  # Expected: None (invalid path)

#Lambda function
print(f"Travel cost for path {path_1} (lambda): {cost_1_lambda} €")  # Expected: Total cost of the valid path
print(f"Travel cost for path {path_2} (lambda): {cost_2_lambda} €")  # Expected: Infinity (invalid path)
